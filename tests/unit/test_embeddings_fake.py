...
async def test_fake_embedding_is_deterministic_and_normalized():
    ...
    a = await provider.embed("create issue github repo")
    b = await provider.embed("create issue github repo")
    assert a == b
...
async def test_similar_texts_have_higher_similarity_than_unrelated():
    provider = FakeEmbeddingProvider(dim=64)
    q = await provider.embed("create issue")
    similar = await provider.embed("create new issue")
    unrelated = await provider.embed("delete pipeline build")
